...
class User_roles extends MY_Controller
{
    function __construct(){
		...
		$this->load->model('user_roles_model', 'user_roles');
    }
	...
	function index(){
		...
		$data['records'] = $this->user_roles->get_all();
	...
	function add(){
		...
		$this->load->view('user_roles/add', $data);
		$this->footer(); 
	}
	...
	function edit($id=""){
		...
		$data['record'] = $this->user_roles->get_role_by_id($id);
		
		$this->header($title = $data['title']); 
		$this->load->view('user_roles/edit', $data);
		$this->footer(); 
	}
	...
	function access($id=""){
		...
		$data['record']= $this->user_roles->get_role_by_id($id);
		$data['access']= $this->user_roles->get_access($id);
		$data['modules']= $this->user_roles->get_modules();

		$this->header($title = $data['title']); 
		$this->load->view('user_roles/access', $data);
		$this->footer(); 
	}

	//-----------------------------------------------------------
	function set_access(){   
		$this->user_roles->set_access();
	}
	...
	public function module(){
		...
		$data['records'] = $this->user_roles->get_all_module();

		$this->header($title = $data['title']); 
		$this->load->view('user_roles/module_list', $data);
		$this->footer();
	}
	...
	public function module_add(){
		...
		if($this->input->post('submit')){
			...
			if ($this->form_validation->run() == FALSE) {
				...
				redirect(base_url('user_roles/module_add'),'refresh');
			}
			else{
				$data = array(
				...
				$result = $this->user_roles->add_module($data);
				if($result){
					$this->session->set_flashdata('success', 'Module has been added successfully!');
					redirect(base_url('user_roles/module'));
				}
			}
		}
		else{
			...
			$this->header($title = $data['title']); 
			$this->load->view('user_roles/module_add', $data);
			$this->footer();
		}
	}
	...
	public function module_edit($id=0){
		...
		if($this->input->post('submit')){
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$this->session->set_flashdata('errors', $data['errors']);
				redirect(base_url('user_roles/module_edit/'.$id),'refresh');
			}
			else{
				$data = array(
					'module_name' => $this->input->post('module_name'),
				...
				$data = $this->security->xss_clean($data);
				$result = $this->user_roles->edit_module($data, $id);
				if($result){
			...
			$data['module'] = $this->user_roles->get_module_by_id($id);

			$this->header($title = $data['title']); 
			$this->load->view('user_roles/module_edit', $data);
			$this->footer();
		}
	}
	...
	public function sub_module($module_id = NULL){
		...
		$data['title'] = '';
		$data['records'] = $this->user_roles->get_sub_module_by_module($module_id);

		$this->header($title = $data['title']); 
		$this->load->view('user_roles/sub_module_list', $data);
		$this->footer();
	}
	...
	public function sub_module_add(){
		...
		if($this->input->post('submit')){
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$this->session->set_flashdata('errors', $data['errors']);
				redirect(base_url('user_roles/sub_module_add/'.$parent),'refresh');
			}
			else{
				$data = array(
					'parent' => $parent,
				...
				$data = $this->security->xss_clean($data);
				$result = $this->user_roles->add_sub_module($data);
				if($result){
					$this->session->set_flashdata('success', 'Sub Module has been added successfully!');
					redirect(base_url('user_roles/sub_module/'.$parent));
				}
			}
		}
		else{
			...
			$this->header($title = $data['title']); 
			$this->load->view('user_roles/sub_module_add', $data);
			$this->footer();
		}
	}
	...
	public function sub_module_edit($id = 0){
		...
		if($this->input->post('submit')){
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$this->session->set_flashdata('errors', $data['errors']);
				redirect(base_url('user_roles/sub_module_edit/'.$id),'refresh');
			}
			else{
				...
				$data = $this->security->xss_clean($data);
				$result = $this->user_roles->edit_sub_module($data, $id);
				if($result){
			...
			$data['title'] = '';
			$data['module'] = $this->user_roles->get_sub_module_by_id($id);
			$this->header($title = $data['title']); 
			$this->load->view('user_roles/sub_module_edit', $data);
			$this->footer();
		}
	}
	...
	public function sub_module_delete($id = 0,$parent = 0){
		...
		$this->user_roles->delete_sub_module($id);

		$this->session->set_flashdata('msg','Sub Menu has been Deleted Successfully.');	
		redirect('user_roles/sub_module/'.$parent);
	}
