...
class Api extends API_Controller
{
   ...
   public function index()
    ...
    public function sendotp_customer(){
				...
				$name = $customer->name;
				$send_otp = send_otp_customer($mobile, $name, $token, $user_type, $customer_id);
	...
	public function verifyotp_customer()
    {
        // API Configuration
				$this->_apiConfig([
					'methods' => ['POST'], // 'GET', 'OPTIONS'
				 ...
				 if(!empty($token))
				$user_verified = verify_otp_customer($mobile, $otp, $token);  
				if($user_verified){   
						// return data
						$this->api_return(
							[
