<?php if(!defined('BASEPATH')) exit('No direct script access allowed');
  ...
  */
  class Datatables
  {
    ...
    * @return mixed
    */
    public function custom_count($valid=0)
    {
      if(!empty($valid) && $valid==1){  
		  $GLOBALS['custom_count_valid'] = 1;
	  }elseif(empty($valid) && $valid!=1){ 
		  $GLOBALS['custom_count_valid'] = 0;
	  }
		...
		foreach($aaData as $key => $value)
		  {
			...
			/////////////// BOOKING LIST
			  ...
			  $aaData[$key]['vehicle_make'] = $this->ci->Common->single_row('vehicle_make',array('make_id'=>$value['vehicle_make']),'make_name');  
			   }
			  if(!empty($value['vehicle_model'])){ 
			  $aaData[$key]['vehicle_model'] = $this->ci->Common->single_row('vehicle_model',array('model_id'=>$value['vehicle_model']),'model_name');  
			  }
			  if(!empty($value['service_category_id'])){ 
			  $aaData[$key]['service_category_id'] = $this->ci->Common->single_row('service_category',array('id'=>$value['service_category_id']),'service_name');  
			  }
			  if(!empty($value['assigned_mechanic'])){ 
			  $aaData[$key]['assigned_mechanic'] = $this->ci->Common->single_row('service_providers',array('id'=>$value['assigned_mechanic']),'name');  
			   }
			
			
			/////////////// FEEDBACK LIST 
			
			  if(!empty($value['feedback_id'])){ 
			  $bookings_data = $this->ci->Common->single_row('bookings',array('booking_id'=>$value['booking_id']));
			  $aaData[$key]['customer_name'] = $bookings_data->customer_name;
			  $aaData[$key]['customer_mobile'] = $bookings_data->customer_mobile;
			  $aaData[$key]['customer_channel'] = $bookings_data->customer_channel;
			  $aaData[$key]['customer_city'] = $bookings_data->customer_city;
			  $aaData[$key]['service_category_id'] = $this->ci->Common->single_row('service_category',array('id'=>$bookings_data->service_category_id),'service_name');  
			  $aaData[$key]['assigned_mechanic'] = $this->ci->Common->single_row('service_providers',array('id'=>$bookings_data->assigned_mechanic),'name');		
			  $aaData[$key]['feedback_id'] = $value['feedback_id'];		
			   }
			  if(!empty($value['feedback_date'])){  
			  $aaData[$key]['feedback_date'] = convert_date($value['feedback_date']); 		
			   }
			
			
			/////////////// COMPLAINTS LIST 
			
			  if(!empty($value['complaints_id'])){ 
			  $bookings_data = $this->ci->Common->single_row('bookings',array('booking_id'=>$value['booking_id']));
			  $aaData[$key]['customer_name'] = $bookings_data->customer_name;
			  $aaData[$key]['customer_mobile'] = $bookings_data->customer_mobile;
			  $aaData[$key]['customer_channel'] = $bookings_data->customer_channel;
			  $aaData[$key]['customer_area'] = $bookings_data->customer_area;
			  $aaData[$key]['service_category_id'] = $this->ci->Common->single_row('service_category',array('id'=>$bookings_data->service_category_id),'service_name');  
			  $aaData[$key]['assigned_mechanic'] = $this->ci->Common->single_row('service_providers',array('id'=>$bookings_data->assigned_mechanic),'name');		
			   
				  
			  $aaData[$key]['complaints_id'] = '<a href="'.base_url().'/complaints/complaints_details/'.$value['complaints_id'].'" class="label label-primary" data-complaints_id="'.$value['complaints_id'].'">'.$value['complaints_id'].'</a>'; 
				  
				$aaData[$key]['booking_id'] = '<a href="'.base_url().'/bookings/booking_details/'.$value['booking_id'].'" class="label label-primary" data-booking_id="'.$value['booking_id'].'">'.$value['booking_id'].'</a>';   
				  
			  $aaData[$key]['created_on'] = convert_date($value['created_on']);		
				  $feedback = $this->ci->Common->single_row('feedback',array('booking_id'=>$value['booking_id']));
				  if(!empty($feedback->feedback)){
				   ...
				   $aaData[$key]['feedback'] = $rating;		
				  
				  $complaints_lifecycle_data = $this->ci->Common->single_row('customer_complaints_lifecycle',array('complaints_id'=>$value['complaints_id']));
				if(!empty($complaints_lifecycle_data->due_date)){ 
					if($complaints_lifecycle_data->due_date!='0000-00-00')
			  		$aaData[$key]['due_date'] = convert_date($complaints_lifecycle_data->due_date);
					else
					$aaData[$key]['due_date'] = ''; 	
				}else{
			 $aaData[$key]['due_date'] = ''; 
			...
			/////////////// LEADS LIST
			if(!empty($value['leads_id'])){ 
				$leads_lifecycle_data = $this->ci->Common->single_row('leads_lifecycle',array('leads_id'=>$value['leads_id']));
				if(!empty($leads_lifecycle_data->due_date)){ 
					if($leads_lifecycle_data->due_date!='0000-00-00')
			  		$aaData[$key]['contact_date'] = convert_date($leads_lifecycle_data->due_date);
					else
					$aaData[$key]['contact_date'] = ''; 	
				}else{
			 $aaData[$key]['contact_date'] = ''; 
				}
				
				if(!empty($value['desired_service_date'])){ 
					if($value['desired_service_date']!='0000-00-00')
			  		$aaData[$key]['desired_service_date'] = convert_date($value['desired_service_date']);
					else
					$aaData[$key]['desired_service_date'] = ''; 	
				}else{
			 $aaData[$key]['desired_service_date'] = ''; 
				} 
					
			  $aaData[$key]['created_on'] = convert_date($value['created_on']);
			  $aaData[$key]['service_category'] = $value['service_category'].'<br>'.get_make($value['make']).' - '.get_model($value['model']);	
			  $aaData[$key]['leads_id'] = '<a href="'.base_url().'/leads/leads_details/'.$value['leads_id'].'" class="label label-primary" data-booking_id="'.$value['leads_id'].'">'.$value['leads_id'].'</a>'; 
			   }
			
			//////////////////// . Department
			
			if(!empty($value['department'])){  
			  $aaData[$key]['department'] = get_department($value['department']); 		
			   }
    ...
    */
    private function get_total_results($filtering = FALSE)
    {
      ...
      $subquery = $this->ci->db->get_compiled_select($this->table);
		...
		if( !empty($GLOBALS['custom_count_valid']) && $GLOBALS['custom_count_valid']==1){ 
		$query = $this->ci->db->query($subquery);
