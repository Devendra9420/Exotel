...
class file_upload
{
	 ...
	 */
	function do_image_upload($file_name, $folder_name = '', $newfilename=NULL)
	{  
		unset($config); 
		...
		$doUPLOAD = $this->ci->upload->do_upload($file_name);	 
        ...
        {  
			$media_url = $this->ci->config->item('media_url', 'file_upload'); 
        	$fInfo = $this->ci->upload->data(); 
			$fInfo['full_path'] = $media_url.$folder_name.'/'.$fInfo['file_name'];
    		return $fInfo;
    ...
    function do_video_upload($file_name, $folder_name = '')
    {
		...
		$this->ci->load->library('upload', $video_config);
