...
class Textlocal
{
	...
	function __construct()
	{
		...
		$settings = get_general_settings();
		 $apiKey = urlencode($settings['textlocal_api']);
		//$username, $hash, $apiKey = false
		 $this->username = $settings['textlocal_username'];
		$this->hash = $settings['textlocal_hash'];
		if ($apiKey) {
			$this->apiKey = $apiKey;
		}
	...
	private function _sendRequest($command, $params = array())
	{
		...
		else return new Exception('Invalid request handler.');
